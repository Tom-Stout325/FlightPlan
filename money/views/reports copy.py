...
FINANCIAL_REPORT_CARDS = [
...

    {
        ...
        "subtitle": "Analyze receipt travel expenses for events.",
...
from ..context_processors import client_profile as brand_context
...
def reports_page(request):
    ...
    def build_cards(card_defs):
        cards = []
        for card in card_defs:
            if enabled_keys is not None and card["key"] not in enabled_keys:
                continue
            c = card.copy()
            c["url"] = reverse(c["url_name"])
            c.setdefault("bg_color", "#f8f9fa")
            c.setdefault("text_class", "text-primary")
            cards.append(c)
        return cards

    ctx = {
        "financial_cards": build_cards(FINANCIAL_REPORT_CARDS),
        "tax_cards": build_cards(TAX_REPORT_CARDS),
        "current_page": "reports",
    }
    ctx.update(brand_context(request))
